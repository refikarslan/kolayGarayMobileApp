...
const carCrashType = ['Evet Kazalı', 'Hayır Arızalı/Problemsiz'];

const carTowStateType = [
  'Evet Yüklenebilir',
  'Hayır Yüklenebilmesi Zor/Vites Boşa Geçmiyor',
];

export class CarTowService extends Component {
  constructor() {
    super();
    this.handleLocCity = this.handleLocCity.bind(this);
    this.handleLocState = this.handleLocState.bind(this);
    this.handleTowSource = this.handleTowSource.bind(this);
    this.handleTowDestination = this.handleTowDestination.bind(this);
    this.handleDescription = this.handleDescription.bind(this);
    this.carTowRequestService = this.carTowRequestService.bind(this);
    ...
    this.state = {
      ...
      description: null,
      towing_source: null,
      towing_destination: null,
      towing_crashed: null,
      towing_block: null,
      towing_execution: 'NOW',
      towing_execution_date: null,
      response: null,
      spinner: false,
      selectedIlce: [],
      selectedDisabled: true,
      selectedGarageCarName: null,
      selectedLocationName: 'Konum Seçildi',
      latitude: null,
      longitude: null,
    };
  }
  ...
  carTowRequestService() {
    if (
      this.state.car_type === null ||
      this.state.location_city === null ||
      this.state.location_state === null ||
      this.state.description === null ||
      this.state.towing_source === null ||
      this.state.towing_destination === null ||
      this.state.towing_crashed === null ||
      this.state.towing_block === null
    ) {
      Alert.alert(
        'Uyarı !',
        ...
        body: JSON.stringify({
          ...
          latitude:
            window.selectedLocation !== null
              ? window.selectedLocation.latitude
              : null,
          longitude:
            window.selectedLocation !== null
              ? window.selectedLocation.longitude
              : null,
          extra: {
            towing_source: this.state.towing_source,
            towing_destination: this.state.towing_destination,
            towing_crashed: this.state.towing_crashed,
            towing_block: this.state.towing_block,
            towing_execution: this.state.towing_execution,
            towing_execution_date: this.state.towing_execution_date,
          },
          commercials: null,
        }),
      })
  ...
  handleTowSource = text => {
    this.setState({towing_source: text});
  };

  handleTowDestination = text => {
    this.setState({towing_destination: text});
  };
  ...
  render() {
    ...
    let selectedLocationName = this.state.selectedLocationName;
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView style={styles.container}>
          ...
          <View style={styles.View0}>
            <Text style={styles.textStandart}>Konum Bilgisi Ekle</Text>
            <TouchableOpacity
          ...
          <View style={styles.View3}>
            <Text style={styles.textStandart}>Araç Kazalı mı?</Text>
            <SelectDropdown
              data={carCrashType}
              onSelect={(selectedItem, index) => {
                if (selectedItem === 'Evet Kazalı') {
                  this.setState({towing_crashed: 1});
                } else {
                  this.setState({towing_crashed: 0});
                }
              }}
              defaultButtonText={'Araç Durumu Seçiniz'}
              buttonTextAfterSelection={(selectedItem, index) => {
                return selectedItem;
              }}
              rowTextForSelection={(item, index) => {
          ...
          <View style={styles.View4}>
            <Text style={styles.textStandart}>
              Aracın çekiciye yüklenebilir mi?
            </Text>
            <SelectDropdown
              data={carTowStateType}
              onSelect={(selectedItem, index) => {
                if (selectedItem === 'Evet Yüklenebilir') {
                  this.setState({towing_block: 0});
                } else {
                  this.setState({towing_block: 1});
                }
              }}
              defaultButtonText={'Araç Çekiciye Yüklenebilme Durumunu Seçiniz'}
              buttonTextAfterSelection={(selectedItem, index) => {
                return selectedItem;
              }}
              rowTextForSelection={(item, index) => {
          ...
          <View style={styles.View5}>
            <Text style={styles.textStandart}>Araç nereden alınacak ?</Text>
            <TextInput
              placeholder=" Örn. Ayrancı Güvenlik Caddesi"
              placeholderTextColor="#808080"
              style={styles.mailInput}
              textContentType="name"
              onChangeText={this.handleTowSource}
              value={this.state.towing_source}
            />
          </View>
          <View style={styles.View5}>
            <Text style={styles.textStandart}>Araç nereye bırakılacak ?</Text>
            <TextInput
              placeholder=" Örn. Şaşmaz Sanayi"
              placeholderTextColor="#808080"
              style={styles.mailInput}
              textContentType="name"
              onChangeText={this.handleTowDestination}
              value={this.state.towing_destination}
            />
          </View>
          <View style={styles.View5}>
          ...
          <View style={styles.signUpButtonView}>
            <TouchableOpacity
              style={styles.button}
              onPress={this.carTowRequestService}>
